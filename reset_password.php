...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($email) || empty($new_password)) {
        $errors[] = "Both email and new password are required.";
    } else {
        // Update the password in the database
        $stmt = $pdo->prepare("UPDATE users SET password = ? WHERE email = ?");
        $stmt->execute([password_hash($new_password, PASSWORD_DEFAULT), $email]);
        
        // Redirect to login.php after successful password reset
        ...
        <div class="container p-8 md:p-10">
            ...
            <form method="POST" action="" class="space-y-6">
                ...
                <div class="input-group">
                    ...
                    <div class="relative">
                        <input type="password" id="new_password" name="new_password" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-transparent" required>
                        <span class="absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer" id="toggleNewPassword">
